...
public:
    ...
    glm::vec3 x = glm::vec3(0.0f, 0.0f, -1.0f);
    glm::vec3 y = glm::vec3(0.0f, 1.0f, 0.0f);
    glm::vec3 position = glm::vec3(0.0f, 1.25f, 2.25f);
...
private:
    ...
    void mainLoop(GLFWwindow * window, Scene & scene) {
        ...
        while( ! glfwWindowShouldClose(window) && !glfwGetKey(window, GLFW_KEY_ESCAPE) ) {
            ...
            time = std::chrono::duration<float, std::chrono::seconds::period>(CT - ST).count();
            
            GLUtils::checkForOpenGLError(__FILE__,__LINE__);
			
            scene.update(float(glfwGetTime()),x,position,y);
            scene.render();
            ...
            //keyboard controls using WASDTV to move respectively with t and v being up and down
            if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS)
            {
                position += camSpeed * time * x;
            }
            if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
            {
                position += camSpeed * time * -glm::normalize(glm::cross(x, y));
            }
    ...
    void updateMatrix(float FOVdeg, float nearPlane, float farPlane)
    {
        ...
        view = glm::lookAt(position, position + glm::vec3(12.0f, 4.0f, 1.0f), y);
